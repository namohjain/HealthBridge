	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String op = DataUtility.getString(request.getParameter("operation"));
		 if (OP_LOG_OUT.equals(op)) {
			...
			ServletUtility.forward(EHCView.LOGIN_VIEW, request, response);
			return;
		}
		if (session.getAttribute("user") != null) {
			ServletUtility.redirect(EHCView.WELCOME_CTL, request, response);
			return;
		}
		...
		if (OP_SIGN_IN.equalsIgnoreCase(op)) {
			...
			try {
				...
				if (bean != null) {
					...
					if(bean.getRoleId()==2) {
						session.setAttribute("doctor", new DoctorModel().findByUserId(bean.getId()));
					}
					
					if(bean.getRoleId()==4) {
						session.setAttribute("patient", new PatientModel().findByUserId(bean.getId()));
					}
					
					if(bean.getRoleId()==3) {
						session.setAttribute("staff", new StaffModel().findByUserId(bean.getId()));
					}
