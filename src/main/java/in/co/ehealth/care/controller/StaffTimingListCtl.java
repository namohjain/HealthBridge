...
@WebServlet(name = "StaffTimingListCtl", urlPatterns = { "/ctl/staffTimingList" })
public class StaffTimingListCtl extends BaseCtl {
	private static Logger log = Logger.getLogger(StaffTimingListCtl.class);

	
	protected BaseBean populateBean(HttpServletRequest request) {
		log.debug("StaffTimingListCtl populateBean method start");
		StaffTimingBean bean = new StaffTimingBean();

		bean.setStaffName(DataUtility.getString(request.getParameter("name")));
		
		log.debug("StaffTimingListCtl populateBean method end");
		return bean;
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		log.debug("StaffTimingListCtl doGet Start");
		List list = null;
		...
		log.debug("StaffTimingListCtl doPOst End");
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		log.debug("StaffTimingListCtl doPost Start");
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					StaffTimingBean deletebean = new StaffTimingBean();
					for (String id : ids) {
		...
		log.debug("StaffTimingListCtl doGet End");

	}
	
	protected String getView() {
		return EHCView.STAFF_TIMING_LIST_VIEW;
	}
}
